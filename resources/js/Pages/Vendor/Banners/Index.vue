<template>
    ...
    <VendorLayout>
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr
                            ...
                            <th class="px-4 py-3 text-start">{{ $t('messages.banner') }}</th>
                            <th class="px-4 py-3 text-start">{{ $t('messages.type') }}</th>
                        ...
                        <tr v-for="banner in banners.data" :key="banner.id"
                            ...
                            <td class="px-4 py-3 text-sm">
                                <div class="flex items-center font-semibold">
                                    <div
                                        class="relative w-16 h-10 rounded-lg ltr:mr-3 rtl:ml-3 bg-gray-100 dark:bg-gray-700 flex items-center justify-center overflow-hidden border">
                            ...
                            <td class="px-4 py-3 text-sm italic">
                                <span class="capitalize">{{ banner.type }}</span>
                            </td>
                            <td class="px-4 py-3 text-xs">
                                <span
                                    :class="banner.active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-700'"
                            ...
                            <td class="px-4 py-3 text-sm">
                                <div class="flex items-center space-x-4 rtl:space-x-reverse">
                                    <button @click="showBanner(banner)"
        ...
        <BannerModal :show="showingModal" :edit-banner="selectedBanner" :store-id="storeId"
            @close="showingModal = false" @refresh="onRefresh" />
        <BannerShow :show="showingShowModal" :banner="selectedBannerToShow" @close="showingShowModal = false" />
    </VendorLayout>
</template>
...
const selectedBannerToShow = ref(null);

const showBanner = (banner) => { selectedBannerToShow.value = banner; showingShowModal.value = true; };
const openModal = () => { selectedBanner.value = null; showingModal.value = true; };
const editBanner = (banner) => { selectedBanner.value = banner; showingModal.value = true; };

const onRefresh = (type) => {
    showingModal.value = false;
    success(type === 'updated' ? 'messages.banner_updated' : 'messages.banner_created');
};

const deleteBanner = (id) => {
    confirmDelete(() => {
        router.delete(route('vendor.banners.destroy', id), { preserveScroll: true });
    });
};
</script>